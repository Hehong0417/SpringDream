...
- (void)setUpBase
{
  ...
  //  product_sku_value数组
    self.tableView.frame = CGRectMake(0, 0, SCREEN_WIDTH, 110);
...
- (void)setUpBottonView
{
    ...
    CGFloat buttonY = self.nowScreenH - buttonH-10;
    for (NSInteger i = 0; i < titles.count; i++) {
        UIButton *buttton = [UIButton buttonWithType:UIButtonTypeCustom];
        [buttton setTitle:titles[i] forState:0];
        buttton.backgroundColor = (i == 0) ? APP_NAV_COLOR : APP_NAV_COLOR;
//        CGFloat buttonX = buttonW * i;
        CGFloat buttonX = 15;

        buttton.tag = i;
        buttton.frame = CGRectMake(buttonX, buttonY, buttonW-30, buttonH);
        [buttton lh_setCornerRadius:3 borderWidth:0 borderColor:nil];
...
- (UIView *)setUpNumbView{
    ...
    UIView *numView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenW, 35)];
    UILabel *numLabel = [UILabel new];
    numLabel.text = @"[数量]";
    numLabel.font = FONT(14);
    numLabel.textAlignment = NSTextAlignmentLeft;
    [self.view addSubview:numLabel];
    numLabel.frame = CGRectMake(10, 0, 80, 35);
