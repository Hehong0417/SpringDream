...
-(instancetype)init{
    ...
    if (self=[super init]) {
        ...
        m_pTextClolor=[UIColor  redColor];
        m_pBackGroundClolor=[UIColor  whiteColor];
...
-(void)yee_MakeBadgeText:(NSString *)text
    ...
    CGSize textSize=[self sizeWithString:text font:tfont constrainedToWidth:self.frame.size.width];
        if ([self isKindOfClass:[UIButton class]]) {
            ...
            [[self  yee_BadgeLable] makeBrdgeViewWithText:text textColor:tColor backColor:backColor textFont:tfont tframe:CGRectMake(weakButton.imageView.frame.size.width*0.5+5+weakButton.imageView.frame.origin.x,weakButton.imageView.frame.origin.y-3, textSize.width+8.0, textSize.height)];
